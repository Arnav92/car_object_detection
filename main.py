#!/usr/bin/env python3
"""
Full Car Detection pipeline with per-epoch speed profiling moved to testing phase.

- Rectangular training (rect=True) and save_period=1 so epoch checkpoints are produced.
- In the testing phase we scan run/weights for epoch checkpoints and measure speed on VAL.
- Results.csv is augmented with per-epoch speed columns (median, p95, mean, fps, throughput, flops).
- Produces PDF with Speed vs Epochs shown below loss/metric curves.
"""
import os
import math
import glob
...
EPOCHS = 20
BATCH_SIZE = 16
IMGSZ = 640  # rectangular training preserves original aspect ratios
VAL_SPLIT = 0.1
RANDOM_SEED = 42
...
def train_one_model(model_name: str, pretrained_path_or_name: str, dataset_yaml_path: str,
                    ...
                    save_period: int = 1) -> Dict[str, str]:
    """
    Train with rectangular images and save_period so epoch checkpoints exist.
    (No per-epoch speed measurement here — moved to test phase.)
    """
    print(f"\n=== Training {model_name} ===")
...
# -------------------------------
# Per-epoch speed measurement moved to test-time
# -------------------------------
def compute_and_write_per_epoch_speed(run_dir: str, val_images_dir: str, imgsz: int = IMGSZ,
                                      warmup: int = 4, max_images: int = None) -> str:
    """
    Scans run_dir/weights for epoch checkpoints and measures speed on val_images_dir.
    Produces run_dir/speed_per_epoch_<run_name>.csv and augments run_dir/results.csv with speed columns.
    Returns path to created csv or None.
    """
    weights_dir = os.path.join(run_dir, "weights")
    if not os.path.exists(weights_dir):
        print(f"No weights directory found for run: {run_dir}")
        return None

    # find checkpoint files and map epoch numbers
    epoch_map = {}
    for p in glob.glob(os.path.join(weights_dir, "*.pt")):
        ...
        if ep is not None:
            epoch_map[ep] = p
    if len(epoch_map) == 0:
        print(f"No epoch*.pt files found under {weights_dir}.")
        return None

    epoch_nums = sorted(epoch_map.keys())
    rows = []
    print(f"Measuring speed for {len(epoch_nums)} epoch checkpoints in {weights_dir} ...")
    for ep in epoch_nums:
        wpath = epoch_map[ep]
        print(f"  epoch {ep} -> {os.path.basename(wpath)}")
        metrics = measure_model_speed_and_flops(wpath, val_images_dir, imgsz=imgsz, warmup=warmup, max_images=max_images)
        median_ms = metrics.get("median_ms")
        fps_m = (1000.0 / median_ms) if (median_ms and median_ms > 0) else None
        rows.append({
            "epoch_ckpt": ep,
            "speed/median_ms": median_ms,
    ...
    speed_df = pd.DataFrame(sorted(rows, key=lambda r: r["epoch_ckpt"]))
    speed_csv = os.path.join(run_dir, f"speed_per_epoch_{os.path.basename(run_dir)}.csv")
    try:
        speed_df.to_csv(speed_csv, index=False)
        print(f"Wrote per-epoch speed CSV: {speed_csv}")
    except Exception as e:
        print(f"Failed to write per-epoch CSV: {e}")
        speed_csv = None
    ...
    if os.path.exists(results_csv):
        try:
            ...
            added_cols = {
                "speed/val_median_ms": "speed/median_ms",
                "speed/val_p95_ms": "speed/p95_ms",
                "speed/val_mean_ms": "speed/mean_ms",
                "speed/val_fps": "speed/fps",
                "speed/val_throughput_fps": "speed/throughput_fps",
                "speed/val_flops_g": "speed/flops_g"
            }
            for dest_col in added_cols.keys():
                if dest_col not in res_df.columns:
                    res_df[dest_col] = np.nan

            # insert speed rows into matching epoch rows (epoch in results.csv typically 0-based)
            for _, row in speed_df.iterrows():
                ep_ckpt = int(row["epoch_ckpt"])
                target_epoch = ep_ckpt - 1  # map 1-based checkpoint to 0-based results epoch
                matches = res_df.index[res_df["epoch"] == target_epoch].tolist()
                if len(matches) > 0:
                    idx = matches[0]
                    res_df.at[idx, "speed/val_median_ms"] = row["speed/median_ms"]
                    res_df.at[idx, "speed/val_p95_ms"] = row["speed/p95_ms"]
                    res_df.at[idx, "speed/val_mean_ms"] = row["speed/mean_ms"]
                    res_df.at[idx, "speed/val_fps"] = row["speed/fps"]
                    res_df.at[idx, "speed/val_throughput_fps"] = row["speed/throughput_fps"]
                    res_df.at[idx, "speed/val_flops_g"] = row["speed/flops_g"]
                else:
                    # append row aligned to epoch
                    newrow = {c: np.nan for c in res_df.columns}
                    newrow["epoch"] = target_epoch
                    ...
                    newrow["speed/val_fps"] = row["speed/fps"]
            ...
            print(f"Augmented results.csv at {results_csv} with per-epoch speed columns.")
        except Exception as e:
            print(f"Failed to augment results.csv: {e}")
    else:
        print(f"results.csv not present at {results_csv}; skipping augmentation.")
...
def generate_pdf_report(per_model_artifacts: Dict[str, Dict[str, str]], out_pdf: str,
    ...
    val_meds = []; val_p95 = []; val_fps = []; val_flops = []
    test_meds = []; test_p95 = []; test_fps = []; test_flops = []
...
def test():
    ...
    print("\nMeasuring speed and FLOPs for each model on validation and test sets (best weights) ...")
    for cfg in MODEL_CONFIGS:
        model_label = cfg["name"]
        weights = per_model[model_label]["best_weights"]
    ...
    # 3) Per-epoch speed: for each run, scan its weights/directory and measure VAL speed per epoch,
    #    then write speed_per_epoch CSV and augment results.csv.
    print("\nComputing per-epoch speed for all runs (VAL set) ...")
    for cfg in MODEL_CONFIGS:
        run_name = cfg["run_name"]
        run_dir = os.path.join(WORKSPACE, "runs", run_name)
        if not os.path.exists(run_dir):
            print(f"Run dir missing: {run_dir} (skipping per-epoch speed for this run)")
            continue
        speed_csv = compute_and_write_per_epoch_speed(run_dir, ds_paths['val_images'], imgsz=IMGSZ, warmup=4, max_images=None)
        if speed_csv:
            per_model[cfg["name"]]["speed_per_epoch_csv"] = speed_csv

    # 4) Write consolidated speed JSON (final summaries + per-epoch val series if available)
    consolidated = {}
        ...
        if art.get("speed_per_epoch_csv") and os.path.exists(art["speed_per_epoch_csv"]):
            try:
                ...
                consolidated[m]["val_by_epoch"] = {
                    "epoch_ckpt": sdf["epoch_ckpt"].tolist() if "epoch_ckpt" in sdf.columns else sdf["epoch_ckpt"].tolist() if "epoch_ckpt" in sdf.columns else sdf.get("epoch_ckpt", []).tolist(),
                    "median_ms": sdf.get("speed/median_ms", pd.Series([None]*len(sdf))).tolist(),
                    "fps": sdf.get("speed/fps", pd.Series([None]*len(sdf))).tolist(),
                    "p95_ms": sdf.get("speed/p95_ms", pd.Series([None]*len(sdf))).tolist(),
                    "mean_ms": sdf.get("speed/mean_ms", pd.Series([None]*len(sdf))).tolist(),
                    "flops_g": sdf.get("speed/flops_g", pd.Series([None]*len(sdf))).tolist()
                }
            except Exception:
                pass
    ...
    for m, art in per_model.items():
        ...
        if 'metrics' in art:
            ...
            print(f"    - Metrics (test): median={art['metrics']['test'].get('median_ms')} ms, p95={art['metrics']['test'].get('p95_ms')}")
...
def main():
    ...
    # train()  # uncomment to retrain (will produce epoch*.pt files if save_period=1)
    test()
